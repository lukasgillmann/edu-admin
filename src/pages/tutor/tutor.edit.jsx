...
import { actionTutorEdit, actionTutorList } from "../../context/action";
...
const TutorEdit = () => {
  ...
  const { page, pageSize, tutorId } = numberInputs(useParams());

  const navigate = useNavigate();

  const [controller, dispatch] = useAsterController();
  const { tutors, loadedTutorCreate } = controller;

  const [url, setUrl] = useState('');
  const [waiting, setWaiting] = useState(false);

  const currTutor = useMemo(() => tutors.page === page && tutors.page_size === pageSize ? tutors.data.find(v => v.id === tutorId) || {} : {}, [page, pageSize, tutors, tutorId]);
  ...
  useEffect(() => setUrl(currTutor.avatar), [currTutor.avatar]);
  useEffect(() => loadedTutorCreate && waiting && navigate('/user/tutors'), [loadedTutorCreate, waiting, navigate]);
  ...
  const onSubmit = (values) => {
    ...
    actionTutorEdit(dispatch, obj, t);
    setWaiting(true);
  };

  const formik = useFormik({
    initialValues: {
      first_name: currTutor.first_name || '',
      last_name: currTutor.last_name || '',
      gender: currTutor.gender || 'm',
      year_of_birth: currTutor.year_of_birth || '2022',
      bio: currTutor.bio || '',
      email: currTutor.email || '',
      phone_number: currTutor.phone_number || '',
      country: currTutor.country || '',
      state: currTutor.state || '',
      city: currTutor.city || '',
    },
  ...
  return <div className="p-4 md:p-8">
    ...
    <PaginationFetcher page={page} pageSize={pageSize} list={tutors} action={actionTutorList} />
    ...
    <form onSubmit={formik.handleSubmit}>
      <Grid container spacing={2} className="md:w-8/12">
        ...
        <Grid item xs={12} className="flex justify-center md:justify-end">
          <VButton variant="contained" color="primary" type="submit" className="h-min text-sm" loading={!loadedTutorCreate}>{t('Save')}</VButton>
          <Link to={`/user/tutors`} className="no-underline ml-4">
