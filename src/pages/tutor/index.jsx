...
import { actionTutorDelete, actionTutorList } from "../../context/action";
...
const Tutor = () => {
  ...
  const { tutors, loadedAdminDashboardGet, loadedTutorList, loadedTutorDelete } = controller;
  ...
  const showList = useMemo(() => {
    ...
    return (tutors.data || []).filter(v => v.first_name?.toLowerCase()?.includes(tmp) || v.last_name?.toLowerCase()?.includes(tmp) || v.email?.toLowerCase()?.includes(tmp));
  }, [tutors.data, term]);
  ...
  const onDelete = (idx) => {
    confirm({
      ...
      description: <div><Icon icon="emojione-v1:warning" />&nbsp;{t("Warning: All data of this tutor will be permanently deleted!")}</div>
    })
      .then(() => actionTutorDelete(dispatch, tutors.data[idx].id, t))
      .catch(() => { });
  };
  ...
  const columns = [
    {
      ...
      cell: rw => <div className="flex items-center" >
        <Link to={`/user/tutors/edit/${tutors.page}/${tutors.page_size}/${rw.id}`} className="no-underline">
          <VAvatar src={rw.avatar ? rw.avatar : ""} className="w-16 h-16" />
        </Link>
        <div className="ml-4">
          <VText div>{rw.first_name} {rw.last_name}</VText>
          <VText color="secondary" className="text-xs bg-gray-100 dark:bg-gray-700 rounded px-1 py-0.5 mt-4 w-max" div>{t("Tutor")}</VText>
        </div>
      </div>,
      grow: 1,
      sortable: true,
  ...
  return <div>
    <div className="px-6 py-8" id="v-user-title">
      <Grid container spacing={2}>
        <Grid item sm={12} md={4} className="flex">
          <VText className="text-2xl">{tutors.total}</VText>&nbsp;
          <VText color="secondary" className="text-2xl">{t("Tutors in total")}</VText>
        </Grid>
        <Grid item sm={12} md={8} className="flex justify-center md:justify-end flex-wrap items-center">
          ...
          <Link to={`/user/tutors/add`} className="no-underline ml-4">
          ...
          <BPopOver className="ml-4" trigger={
            ...
            <div className="flex flex-col">
              <VButton variant="text" color="secondary" className="px-4 py-1 justify-start w-full" onClick={() => downloadCSV(exportColumns, tutors.data, "tutors")}>
                <Icon icon="ph:file-csv" />&nbsp;{t("Export CSV")}
              </VButton>
              <VButton variant="text" color="secondary" className="px-4 py-1 justify-start w-full" onClick={() => downloadHTML(exportColumns, tutors.data, "tutors")}>
    ...
    <div>
                    ...
                    <div className="border border-solid border-gray-300 rounded-md">
                      <div className="px-4 py-2 flex items-center">
                        <VText color="secondary" className="text-xs bg-gray-100 dark:bg-gray-700 rounded px-1 py-0.5 w-max flex items-center h-min">
                          <Icon icon="ic:outline-published-with-changes" />&nbsp;{t("Tutor")}
                        </VText>
                      ...
                      <Divider className="bg-gray-200 dark:bg-gray-500" />
                      <div className="p-4">
                        <Link to={`/user/tutors/edit/${tutors.page}/${tutors.page_size}/${tutors.data[idx].id}`} className="no-underline">
      ...
      {
        tab === 1 && <BTable
          key={`${loading}${loadedTutorDelete}${showList.length}`}
          columns={columns}
          data={showList}
          total={tutors.total}
          loading={loading}
    ...
    <PageNoData show={!loading && !tutors.total} />
