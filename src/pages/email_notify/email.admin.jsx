...
const EmailAdmin = () => {
  ...
  useEffect(() => emailTemplates.length && setUserTemplates(emailTemplates.filter(v => v.end_user === 'admin')), [emailTemplates]);
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (rw, idx) => <Link to={`/email/edit/admin/${idx}`} className="no-underline">
  ...
  return <div>
    <div className="p-4 md:p-8">
      <VText className="text-2xl" div>{t('Administrator')}</VText>
      <VText color="secondary">{t('List of email notifications for the administrator role')}</VText>
    </div>
                ...
                showList.map((rw, idx) =>
                  <Grid key={rw.id} item xs={12} sm={6} lg={4} xl={3}>
                    <div className="border border-solid border-gray-300 rounded-md">
                      ...
                      <Divider className="bg-gray-200 dark:bg-gray-500" />
                      <div className="p-4">
                        <Link to={`/email/edit/admin/${idx}`} className="no-underline">
