...
import { actionCourseList, actionGroupAssignCourses } from "../../context/action";
import { getCourseImageUrl, getFormatDate } from "../../utils/string";
import toaster from "../../utils/toast.msg";

const GroupDetailProgramAdd = (props) => {

  const { t, i18n } = useTranslation('common');

  const { groupId, groupCourses, disabled, userCount } = props;

  const [modalOpen, setModalOpen] = useState(false);
  const [selectCourses, setSelectCourses] = useState([]);

  const [controller, dispatch] = useAsterController();
  const { courses, loadedAdminDashboardGet, loadedCourseList, loadedGroupCourseAssign, dashboardInfo } = controller;
  ...
  const onAssign = () => {
    ...
    if (!userCount) {
      toaster({ type: "error", title: t("There is no user in this group!"), body: t("Please add users first, and then assign courses!") });
      return;
    }

    if (selectCourses.length * userCount > licenseNumber.license_available) {
      toaster({ type: "error", title: t("License limit error!"), body: t("Please contact your sales manager to update your licenses credits!") });
      return;
    }

    actionGroupAssignCourses(dispatch, { group_id: groupId, courses: selectCourses }, t);
    setModalOpen(false);
  };
  ...
  return <>
    ...
    <VButton variant="contained" color="primary" className="text-sm ml-2 normal-case" onClick={() => { setSelectCourses([]); setModalOpen(true); }} disabled={disabled}>
    ...
    <BModal open={modalOpen} setOpen={setModalOpen} width='90vw' height='fit-content' className="w-full max-w-xl h-full max-h-150">
      ...
      <div className="px-8 pt-8 flex justify-between">
        ...
        <VButton variant="contained" color="primary" className="text-sm ml-2 normal-case" onClick={onAssign} disabled={!selectCourses.length} loading={!loadedGroupCourseAssign}>
          {t("Assign")}
        </VButton>
      </div>
      ...
      <BTable
        ...
        selectableRowDisabled={row => groupCourses.map(v => v.id).includes(row.id)}
        page={courses.page}
...
export default GroupDetailProgramAdd;